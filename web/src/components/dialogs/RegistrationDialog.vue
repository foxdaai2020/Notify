<template>
  <v-dialog v-model="openRegistrationDialog" persistent width="350px">
    <v-card height="560px">
      <v-toolbar>
        <v-icon color="DarkRed" class="mr-2">mdi-comment-account</v-icon>
        <v-toolbar-title style="font-size: 16px; color: #696969;"
          >Registration: Users</v-toolbar-title
        >
        <span
          style="position: absolute; height: 0px; left: 0px; right: 0px; top: 54px; border: 0.5px solid #F4F4F4;"
        ></span>
        <v-spacer></v-spacer>
        <v-btn
          color="transparent"
        ...
        <template v-slot:extension>
          <v-btn outlined color="FountainBlue White--text" class="mb-2"
            >Download Failed Users</v-btn
          >
          <div
            style="position:absolute; top:50px; height: 380px; overflow: scroll;"
          >
            <table>
              <thead>
                <tr>
  ...
  export default {
    ...
    data() {
      return {
        ...
        tab: null,
        items: ["User Information", "Reset Password"],
        text:
          "Lorerud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
      };
    },
    methods: {
      closeDialog() {
